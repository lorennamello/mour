...
**Direcionadores clássicos do modelo (a mesma lista de critérios do seu material de referência):**

- **Número de clientes:** baixo no segmento que a Moura tentou primeiro — só 3 grandes operadoras nacionais (Claro, TIM, Vivo). Poucos compradores concentrando quase toda a receita endereçável é, por si só, um fator que aumenta o poder de cada um deles. No segmento de ISPs regionais essa lógica se inverte: 11.853 empresas pulverizadas, nenhuma com peso individual.
- **Tamanho do pedido de cada cliente:** grande — um contrato fechado com uma operadora nacional cobre potencialmente milhares de sites de uma vez. Isso é uma faca de dois gumes: um contrato fechado muda o jogo da Moura, mas também significa que perder a decisão de uma única operadora (como aconteceu duas vezes) custa uma fatia desproporcional do mercado endereçável.
- **Diferenças entre concorrentes (do ponto de vista de quem compra):** baixas — como a força de Rivalidade mostrou, a qualidade técnica virou piso entre os fornecedores sérios e a interoperabilidade elimina diferenciação técnica. Quanto mais parecidas as ofertas, mais fácil é para o comprador trocar sem perder nada — e mais poder ele tem.
- **Sensibilidade ao preço:** altíssima — foi o único fator que decidiu os dois casos documentados, mesmo com elogio técnico explícito ao produto.
- **Habilidade do comprador em substituir (fazer sozinho):** alta nas grandes operadoras — a Claro já constrói plataforma própria agnóstica; ambas já gerenciavam energia internamente antes do piloto e voltaram a isso depois.
- **Disponibilidade de informação de compradores:** alta — Claro cotou 4 fornecedores nos mesmos critérios, TIM rodou piloto formal comparando 2 fornecedores lado a lado nos mesmos indicadores; ambas têm times técnicos internos capazes de auditar tecnicamente qualquer proposta, não dependem do vendedor para entender o que estão comprando.
- **Mudança de custos:** baixa, pelos mesmos motivos do bloco de perguntas-guia acima (interoperabilidade + serviço por SLA).

> **Troca de fornecedor: além da bateria, a visão da solução energética completa.** Até aqui, a evidência de "troca fácil" vem principalmente da bateria (interoperabilidade via SNMP). Mas a oferta da Moura não é só a célula — é um pacote de quatro camadas, e a facilidade de troca não é igual em todas:
> 1. **Bateria** — fácil de trocar, comoditizada tecnicamente (confirmado pela TIM).
> 2. **Gabinete/estrutura física** — não é algo que o cliente troca separadamente; ele vem embutido no pacote de quem for escolhido como fornecedor. Ou seja: o cliente não "troca o gabinete", ele troca o fornecedor inteiro, e o gabinete vai junto. (Do lado de dentro da Moura, isso é diferente — ver Força 3: só existe 1 fornecedor homologado de gabinete no Brasil, uma vulnerabilidade da própria Moura como fabricante, não uma barreira para o cliente trocar.)
> 3. **Software/monitoramento** — mais fácil de trocar do que a própria bateria: a Claro construiu deliberadamente uma plataforma própria, agnóstica de fabricante, exatamente para não depender do software de nenhum fornecedor específico. O cliente nem precisa mais do painel do fornecedor escolhido.
> 4. **Serviço de instalação e manutenção (O&M)** — já é contratado à parte, por SLA e penalidade, sem vínculo com o fabricante do equipamento — mais um ponto de baixíssimo custo de troca.
> 5. **Modelo contratual (comprar vs. alugar)** — este é o único ponto real de atrito. Um contrato de aluguel plurianual de fato prenderia o cliente a um fornecedor por anos. E é exatamente por isso que as duas operadoras recusaram esse modelo: preferem seguir comprando equipamento avulso (baixo custo de troca em todas as outras camadas) a assinar um compromisso de longo prazo com um único fornecedor (o único lugar onde o custo de troca seria realmente alto).
>
> Leitura: o comprador não está só evitando pagar mais — está deliberadamente mantendo baixo o custo de troca em toda a cadeia (bateria, software, serviço), e o único ponto onde a Moura conseguiria de fato reter o cliente é justamente o modelo contratual que ele está recusando assinar.
...
**Direcionadores clássicos do modelo:**

- **Número de concorrentes:** alto — ~20 mapeados no benchmark interno mais outras seis frentes no relatório de ISPs (26+ nomeados).
- **Diversidade de concorrentes:** alta — quatro arquétipos muito diferentes entre si (bateria, operação, infraestrutura, plataforma) mais startups de assinatura, fabricantes de nobreak CAPEX, locadoras de diesel, distribuidoras e integradores solares. Não é um grupo competindo pela mesma proposta de valor.
- **Concentração de mercado:** baixa-média no núcleo de EaaS (ninguém domina ainda), mas alta em segmentos adjacentes que podem verticalizar — as 4 maiores towercos (American Tower, Highline, SBA, Sitios LatAm) já concentram 63,68% das torres do país.
- **Crescimento do mercado:** rápido (detalhado acima) — o que reduz um pouco a pressão de "briga pelo mesmo pedaço", já que ainda há espaço para vários players crescerem sem necessariamente roubar fatia um do outro.
- **Diferenças de qualidade:** baixas entre os players "sérios" (Moura, UCB Power, Polarium, EnerSys, Huawei) — qualidade virou piso, não diferencial, como Fabiano confirma ao elogiar a Moura mesmo recusando por preço. Ainda há diferença real frente a concorrentes de baixo custo ainda não homologados.
- **Lealdade à marca:** baixa — nenhuma evidência de lealdade nas decisões documentadas; as decisões foram 100% por número financeiro, e as operadoras testam múltiplos fornecedores lado a lado deliberadamente para não depender de marca nenhuma.
- **Barreiras à existência (de saída):** médias-altas para quem já fabricou e capitalizou — o próprio Diagnóstico aponta que o modelo de aluguel "exige imobilizar dezenas de milhões de reais em fábrica, baterias e gabinetes antes de começar a receber as parcelas". Isso é uma barreira de saída para a própria Moura: uma vez capitalizada nesse modelo, sair do jogo é caro, o que tende a manter concorrentes competindo mesmo com margem apertada.
- **Mudança de custos:** baixa para o comprador trocar de concorrente, como já visto na Força 1.
...
> **Quem é o único fornecedor homologado de gabinete no Brasil?** Nenhum dos materiais que temos — nem o Diagnóstico Fase 1, nem as transcrições internas — nomeia essa empresa. O Diagnóstico só descreve a situação ("há apenas uma opção viável de fornecedor homologado no Brasil para fabricar a estrutura metálica dos gabinetes") sem citar o nome, e nas entrevistas internas o fornecedor aparece sempre de forma genérica: Karina (Comercial) diz *"eu não tenho uma opção China, duas, três opções China"* ao falar do gabinete, e Yasmin (Engenharia) se refere a ele apenas como *"nosso fornecedor... de gabinete"* ao comentar as melhorias no cadeado antifurto. Ou seja: o material disponível confirma **que existe essa concentração de fornecedor único**, mas não revela **quem é essa empresa** — provavelmente porque é uma informação interna de Compras/Suprimentos que não apareceu nas entrevistas que tivemos acesso. Se você precisar do nome exato para a apresentação, isso precisa vir de uma conversa direta com o time de Compras/Operações da Moura — não está em nenhum documento desta análise.

**Direcionadores clássicos do modelo:**

- **Número e tamanho dos fornecedores:** já respondido acima de forma polarizada por insumo (1 para gabinete, muitos para célula, poucos para capital).
- **Singularidade dos produtos de cada fornecedor:** baixa para célula (commodity, muitos substitutos); alta para gabinete (nenhum substituto homologado); média para capital (dinheiro é fungível, mas a linha compatível com o prazo do modelo de aluguel é escassa).
- **Habilidade da Moura em substituir esse fornecedor:** baixa para gabinete (não há para quem trocar hoje); alta para célula (excesso de oferta); baixa para capital (poucas linhas adequadas a esse perfil de risco/prazo, como o BNB).
...
- **Barreiras à entrada:** mistas, como já detalhado — altas em capital, baixas em tecnologia e conquista de cliente.
- **Economias de escala:** favorecem incumbentes de capital pesado (Huawei, EnerSys, fabricantes globais) e prejudicam pequenos entrantes que precisariam diluir o custo de capital de giro do modelo de aluguel — mas favorecem entrantes "leves" que não fabricam nada e apenas financeirizam equipamento de terceiros, como as startups de EaaS.
- **Lealdade à marca:** baixa, como já visto — não é barreira relevante nas decisões documentadas.
- **Requisitos de capital:** altos para o modelo de assinatura de longo prazo; baixos para quem vende no modelo CAPEX tradicional — qualquer integrador solar regional já faz isso hoje.
- **Experiência cumulativa (curva de aprendizado):** baixa como barreira — Claro testando 4 fornecedores e TIM testando 2 em pouco tempo mostra que não é preciso décadas de histórico para entrar na mesa de decisão.
- **Políticas de governo:** neutras a favoráveis — nenhuma barreira regulatória forte aparece nas fontes; ao contrário, linhas de crédito subsidiadas (como o BNB) tendem a facilitar a entrada de novos modelos de financiamento de energia.
- **Canais de acesso à distribuição:** relativamente abertos — operadoras cotam diretamente com múltiplos fornecedores, sem um canal fechado protegendo incumbentes; towercos e redes neutras já têm relação direta com o cliente final via seus próprios contratos de locação, o que lhes dá um canal de distribuição pronto para embutir energia.
- **Mudança de custos:** baixa para o cliente aceitar um novo entrante, pelos mesmos motivos já discutidos nas Forças 1 e 2.
...
> **Como isso substitui a solução de energia da Moura, na prática?** A pergunta certa não é "essa tecnologia é melhor que bateria" — é "dá pra resolver o mesmo problema do cliente sem comprar ou alugar mais energia?".
>
> O problema que o EaaS da Moura resolve é: *"quando a energia da concessionária cai, por quanto tempo o site continua funcionando com a bateria que tem?"* A resposta padrão da indústria até hoje é aumentar a **oferta** de energia — comprar ou alugar mais capacidade de bateria para ganhar mais horas de autonomia.
>
> Fabiano descreve uma resposta alternativa que ataca o outro lado da equação: reduzir a **demanda** durante a crise, em vez de aumentar a oferta. O exemplo dele: uma torre roda simultaneamente 2G, 3G, 4G e 5G, todas consumindo energia da mesma bateria ao mesmo tempo. Se a energia da rua cai e a bateria começa a descarregar, o sistema pode — por lógica de software, sem trocar nenhum equipamento — ir desligando as camadas menos essenciais em sequência (5G primeiro, depois 3G/2G), deixando ligado só o 4G, que já cobre voz e dados. Na fala dele, essa única mudança de lógica pode transformar *"uma hora de backup de energia em 4 horas"* — usando exatamente a mesma bateria que já está instalada, sem comprar nem alugar nada a mais.
>
> É por isso que isso conta como **substituto** e não como um concorrente a mais: nenhuma empresa de energia — nem a Moura, nem a UCB Power, nem ninguém do benchmark — vende esse tipo de solução. Quem resolve isso é o próprio time de engenharia de rede da operadora, mudando como a rede se comporta, não quem fornece bateria. Se a Claro consegue multiplicar por 4 a autonomia da bateria que já tem só reconfigurando a rede, ela reduz (ou elimina) exatamente a necessidade que levaria o cliente a contratar mais capacidade de Energia como Serviço da Moura — o mesmo problema é resolvido de graça, por engenharia, sem nenhum investimento em energia.
...
- **Número de produtos substitutos disponíveis:** 4 (gestão própria, diesel, redesenho de demanda via software, internalização via M&A).
- **Propensão do comprador à substituição:** alta — nos dois casos documentados o comprador de fato migrou de volta para o substituto (gestão própria) depois de testar o produto novo.
- **Desempenho de preço relativo do substituto:** favorável ao substituto no horizonte de decisão que as operadoras usam (payback de até 3 anos) — é exatamente por isso que ele venceu nos dois casos.
- **Valor percebido da diferenciação do produto da Moura:** alto tecnicamente (elogios explícitos de Fabiano e Vinícius) mas baixo financeiramente — a diferenciação não se traduziu em disposição de pagar mais ou comprometer capital de longo prazo.
- **Mudança de custos:** baixíssima — voltar para a gestão própria não exige nenhum investimento novo, é a opção "natural" de não fazer nada diferente do que já era feito.
