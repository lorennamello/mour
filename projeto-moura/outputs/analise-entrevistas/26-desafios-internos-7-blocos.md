...
**Pontos a investigar** *(o que fazer para decidir com dado — não a resposta pronta; o oposto do desafio não é automaticamente o caminho certo)*:
- **Sobre a customização sem padrão:** mapear, contrato a contrato, o que da customização atual (engenharia, precificação, condições contratuais) o cliente de fato valoriza e pagaria de novo por isso, versus o que é customização por hábito, sem ganho percebido — só com esse mapa dá pra decidir o que padronizar e o que manter sob medida.
- **Sobre o playbook único (Vivo):** testar o modelo comercial da Vivo com um segundo cliente real (não hipotético) para descobrir se o que funcionou foi o modelo em si, replicável, ou as circunstâncias específicas daquele cliente (timing, perfil, histórico de relação).
- **Sobre "aluga a caixa d'água, não garante o banho":** investigar diretamente com clientes (atuais e prospects) se a diferença entre "backup por tempo determinado" e "energia sempre disponível" já pesa na decisão de compra ou na percepção de risco deles — antes de decidir se vale mudar o que o contrato garante ou só comunicar melhor o que já é garantido.
- **Sobre o escopo restrito (sem rede elétrica pública, ar-condicionado, gerador):** revisitar essa decisão de forma deliberada com o que já se sabe de demanda de mercado hoje — reconfirmando o escopo atual ou ampliando-o, mas por decisão, não por inércia de uma escolha antiga.
- **Sobre a resistência a "quebrar o pacote":** investigar o que, dentro da oferta completa (bateria + gabinete + monitoramento + serviço), o cliente realmente valoriza isoladamente — versus o que ele só valoriza porque vem junto com o resto. Isso é o que permite decidir se compensa abrir o pacote, e para quais partes.
- **Sobre o argumento comercial não convencer:** testar separadamente as duas hipóteses possíveis — (a) apresentar o argumento de custo de oportunidade de forma mais robusta e quantificada, pra ver se muda a decisão do comprador; ou (b) confirmar que o comprador simplesmente não usa esse critério hoje, e nesse caso vender pela língua que ele já usa (hard saving) em vez de tentar convertê-lo a um critério novo.
