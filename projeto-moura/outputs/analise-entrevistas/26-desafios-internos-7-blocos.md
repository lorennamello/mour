...
**Oportunidades** *(a pergunta em aberto que cada desafio revela — não a solução pronta; o oposto do desafio não é automaticamente o caminho certo)*:
- **Sobre a customização sem padrão:** hoje a customização acontece por padrão, não por escolha — ninguém decidiu deliberadamente até onde vale padronizar e até onde a customização é o que de fato fecha negócio. A oportunidade não é necessariamente "padronizar" — é primeiro separar, caso a caso, o que da customização atual gera valor real pro cliente (e pode até virar diferencial competitivo) do que é só custo operacional evitável. Só depois disso decidir o que fazer.
- **Sobre o playbook único (Vivo):** ainda não se sabe se o modelo Vivo é replicável ou se foi um caso que funcionou por circunstâncias específicas dele (timing, perfil do cliente, histórico de relação). A oportunidade é testar isso com um segundo caso real antes de assumir qualquer direção — "isso se repete" e "cada operadora precisa de um modelo próprio" são as duas hipóteses em aberto, nenhuma é fato ainda.
- **Sobre "aluga a caixa d'água, não garante o banho":** existe uma lacuna entre o que o contrato garante (tempo de backup) e o que o cliente talvez espere (energia sempre disponível). Não se sabe se essa lacuna já é um motivo real de hesitação do cliente, ou se é uma preocupação mais forte dentro da Moura do que no mercado. Vale entender isso primeiro — porque "garantir energia o tempo todo" pode ser inviável operacional e financeiramente; a resposta pode não ser ampliar a garantia, pode ser só comunicar melhor o que já é garantido, ou outra coisa que ainda não apareceu.
- **Sobre o escopo restrito:** essa decisão (não incluir rede elétrica, ar-condicionado, gerador) foi tomada uma vez e nunca foi revisitada com dado novo. A oportunidade é reabrir essa decisão conscientemente — o que pode perfeitamente confirmar que o escopo atual está certo, não necessariamente levar a ampliá-lo.
- **Sobre a resistência a "quebrar o pacote":** não está claro se o pedido de modularidade (ex.: só monitoramento) é uma demanda real e recorrente, ou um pedido pontual de um cliente específico. A oportunidade é entender a extensão real dessa demanda antes de decidir se vale abrir mão do pacote fechado — "não vale" também seria um resultado válido dessa investigação, não só "sim, modularizar".
- **Sobre o argumento comercial não convencer:** não está claro se o argumento de custo de oportunidade não convence porque o comprador genuinamente não valoriza isso, ou porque a Moura ainda não sabe provar/quantificar isso de forma convincente. São dois problemas diferentes, com caminhos diferentes — vale isolar qual dos dois é o real antes de investir em qualquer um.
